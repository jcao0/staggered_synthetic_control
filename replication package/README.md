...
This repository provides replication files for Cao, Lu, and Wu, "Synthetic Control Inference for Staggered Adoption."
...
We certify that the author(s) have documented permission to redistribute and publish the data contained in this replication package. The empirical data are publicly available from Harvard Dataverse, and no restricted or confidential data are included.
...
2.  Compute smallest eigenvalues of the sample analogue of the design matrices (MATLAB).
    ...
    -   In MATLAB, run `calculate_eigenvalue_for_design_matrix.m` from the `smallest_eigenvalues/` folder.
...
4.  Estimate treatment effects (R and MATLAB).
    ...
    -   In MATLAB, run `plot_results.m` from the `treatment_effect/` folder.
...
-   `raw_data/` — Contains the raw data for all analyses.
    ...
    -   `psrm_cartel_data_raw.csv` — Non-proprietary CSV copy of `psrm_cartel_data.RData`.
...
-   `treatment_scheme/` — Contains code and data that produce Figure 2, which visualizes the timing of treatment adoption.

    -   `cleaned_data/` — Cleaned outcome data produced by `data_cleaning.R`.
        -   `psrm_cartel_data.csv` — Cleaned dataset for cartel outcomes.
        -   `psrm_crime_data.csv` — Cleaned dataset for homicide and theft outcomes.
    -   `plot_treatment_scheme.R` — Plot the treatment scheme for different outcome groups and save it as Figure 2.
...
-   `treatment_effect/` — Contains data and code to estimate treatment effects and to plot Figure 3.
    ...
    -   `estimation_ssc.m` — MATLAB script that estimates treatment effects by SSC method and saves results.

    -   `estimation_gsc.R` — R script that estimates treatment effects by GSC method and saves results.

    -   `plot_results.m` — MATLAB script that reads SSC and GSC results and produces Figure 3 that compares the methods.
