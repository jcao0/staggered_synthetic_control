...
## Function to create simulated samples
## Revised based on  Yiqing Xu (2017)
...
get_treatment_matrix <- function(N, S, theta=c(-0.0026, 1.037), r=0, seed=NULL) {
    ...
    A <-array(0,dim=c(S,N))    # regressor matrix, must be T by N  by  p 
    for (s in 1:S) {
...
simulate_data_fm <- function(N, T0, S, te=NULL, D.sd=0, A=NULL, theta = c(-1.7,-0.6),
                   p=0, beta=NULL, fsize=1, r=0, FE=FALSE, AR1=0, mu=0, fixF=FALSE, fixL=FALSE, F=NULL, L=FULL,
                    seed=NULL){
    ## N：number of units
    ## T0, S: pre-treatment and post-treatment periods
    ...
    if (p!=0) {
        ...
        for (j in 1:p) {
            X[,,j] <- matrix(rnorm(T*N),T,N)  +
                0.5 * factor[,1] %*% t(lambda[,1]) +
                0.25 * matrix(1,T,1) %*% t(lambda[,1]) +
                0.25 * factor[,1] %*% matrix(1,1,N)+1
        }
    }  

    ## treatment assignment
    D_pre <- matrix(0, nrow = T0, ncol = N)  # pre-treatment matrix 
    if (is.null( A )==TRUE) {
        if(theta==NULL){
        ...
        A = get_treatment_matrix(N=N, S=S, theta = theta, r=l)    # regressor matrix, must be T by N  by  p 
        }
    D = rbind(D_pre, A )  # full treatment matrix
    N_tr<- sum(colSums(D)>0) # number of treated units

    ## event-time treatment indicator
    D.event<-matrix(0,T,N)
    for (s in 1:S) {
        D.event[T0+s,]<-D.event[(T0+s-1),]+D[T0+s,]
    }
    ...
    eff <- matrix(0, T, N)
    for (s in 1:S) {
    eff[T0 + s, D.event[T0 + s, ] > 0] <- te[D.event[T0 + s, D.event[T0 + s, ] > 0]] + rnorm(sum(D.event[T0 + s, ] > 0),0,D.sd)
    }
    ...
    panel<-as.data.frame(cbind(rep(101:(100+N),each=T),rep(1:T,N),
                               ...
                               rep(mu,T*N),rep(1,T*N),rep(N_tr,T*N),rep(att,T*N)))
...
# example usage:
# theta <- get_logistic_theta(r_treat = 0.9, r_treat_period = 0.5, seed=123)
# A <- get_treatment_matrix(N=20, S=10, theta=c(-4,-2.4), r=2)
# sim_data <- simulate_data_fm(N=20, T0=5, S=10, p=0, r=2, AR1=0.5, D.sd=0, A=A)

# another theta choice with small eventually treated units = c(-4,-2.4)