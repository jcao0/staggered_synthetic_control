...
-   `functions/` — Contains R and MATLAB functions used in simulations:
    -   ``` att_ci.m ,``att_event_ci.m ``` — MATLAB functions to implement SSC for overall ATT effect and event-time ATT effect
    -   `ssc_inference.m` — MATLAB functions to implement SSC inference
    -   `ssc.m`, — MATLAB functions to implement SSC estimation of individual treatment effect
    -   `synthetic_control.m`,`synthetic_control_batch.m`— MATLAB functions estimate synthetic control weights for one unit and a batch of units (i.e., all units)
...
-   R (4.5.1)
    -   required packages: R.matlab, doParallel, foreach, MASS, nleqslv, dplyr, parallel, gsynth, augsynth, did, ggplot2, patchwork
-   MATLAB (R2018b)
...
2.  **Run Simulations:**
    ...
    -   Use the MATLAB script (`sim_scs.m`) for SSC results and R scripts (`sim_alternative_methods.R`) for alternative synthetic control variants.
